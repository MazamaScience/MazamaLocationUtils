context("table_findOverlappingDistances")

test_that("bad/missing input generates an error", {
  
  # tbl not a dataframe
  expect_error(table_findOverlappingDistances(locationTbl = c("vector", "instead"), radius = 10))
  # radius not numeric
  expect_error(table_findOverlappingDistances(locationTbl = wa_airfire_meta, radius = "bad radius"))
  
  # tbl missing
  expect_error(table_findOverlappingDistances(tbl <-NULL, radius = 10))
  # radius missing
  expect_error(table_findOverlappingDistances(locationTbl = wa_airfire_meta, radius = NULL))
...
test_that("table missing latitude or longitude generates an error", {
  ...
  expect_error(table_findOverlappingDistances(locationTbl = testTbl, radius = 10))
...
test_that("passing radius = 1000 returns empty tibble", {
  ...
  result <- table_findOverlappingDistances(wa_airfire_meta, radius = 1000)
...
test_that("passing radius = 2000 returns expected output", {
  ...
  result <- table_findOverlappingDistances(wa_airfire_meta, radius = 2000)
