...
Right now, our _known locations_ table only has automatically generated
spatial data:
...
Perhaps we would like to import some of the original metadata over to our new 
...
The whole point of a know location service is to speed up access to spatial
...
Before using **MazamaLocationUtils** you must first install **MazamaSpatialUtils**
and the accompanying spatial datasets.

Once that is in place, the easiest way to set things up is with 
`mazama_initialize()` which is just a wrapper for:
...
so you can experiment without losing your work. And file sizes are pretty tiny
